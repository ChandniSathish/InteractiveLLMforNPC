...
🎙Microphone Input: Speak into your microphone, and your voice is converted to text.
...
Facial Animation and Speech Generation: The LLM's response is converted to speech using text-to-speech. Then a facial animation video is generated, using the extracted face image of the NPC with the audio.

Integration with the Game: The facial animation video and audio are seamlessly integrated into the game by replacing the displayed face pixels with the generated facial animation, making it appear as if the NPC is speaking naturally.
...
```sh
git clone https://github.com/ChandniSathish/InteractiveLLMforNPC.git
```
